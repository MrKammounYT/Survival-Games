	...
	public boolean onCommand(CommandSender sender, Command arg1, String arg2, String[] args) {
		if(sender instanceof Player) {
			...
			if(args.length == 1) {
				...
				else if(tact.equalsIgnoreCase("ForceStart")) {
					if(main.getState(Gstate.waiting)) {
						if(GameState.GamePlayer.size() >= 2) {
					main.forcestart = true;
					p.sendMessage(main.Prefix + "§astarting the game");
						}else {
							p.sendMessage(main.Prefix + "§cYou Need at least 2 Player In the Game To Force Start");
						}
					}else {
						p.sendMessage(main.Prefix + "§cGame Alerday In starting Phaze");
