	...
	public void onchat(PlayerChatEvent e) {
		...
		if(main.getState(Gstate.finish)) {
			if(GameState.spec.contains(p.getName())) {
				e.setFormat(main.color("&7│ §cSpectate §7│ &8" + Titles.getPlayerName(p) + " &7⋙ &f" + e.getMessage()));
			}else {
				e.setFormat(Titles.getPlayerName(p) + " §7⋙ §f" + e.getMessage());
		...
		if(GameState.spec.contains(p.getName())) {
			...
			for(int i=0;i<GameState.spec.size();i++) {
				...
				ps.sendMessage(main.color("&7│ §cSpectate &7│ &8" + Titles.getPlayerName(p) + " &7⋙ &f" + e.getMessage()));
			}
			return;
		}
		int point= points.getpoints(p.getUniqueId());
		if(main.getState(Gstate.waiting) || main.getState(Gstate.starting)) {
			e.setFormat(main.color("&e"+point+" &7│ " + Titles.getPlayerName(p) + " &7⋙ &f" + e.getMessage()));
		}else {
			e.setFormat(main.color(Titles.getPlayerName(p) + " &7⋙ &f" + e.getMessage()));
			}
